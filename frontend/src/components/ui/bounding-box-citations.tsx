      ...
      <div className="mb-3 flex min-h-8 items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-1">
          {isExpectedEditable && modified ? (
            <Tooltip>
              <TooltipTrigger render={<Button type="button" variant="ghost" size="icon-sm" className="text-muted-foreground" onClick={onUndo} aria-label={`Undo ${field.key}`} />}><HugeiconsIcon icon={Undo02Icon} className="size-4" /></TooltipTrigger>
              <TooltipContent>Revert changes</TooltipContent>
            </Tooltip>
          ) : null}
          {isExpectedEditable ? (
            <Tooltip>
              <TooltipTrigger render={<Button type="button" variant="ghost" size="icon-sm" className="text-muted-foreground" onClick={onSetNull} aria-label={`Set ${field.key} to null`} />}><HugeiconsIcon icon={CancelCircleIcon} className="size-4" /></TooltipTrigger>
